...
<form action="{{ route('admin.chapter.update', $chapter) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <select name="volume_id" id="volume_id">
            @foreach ($volumes as $volume)
                <option value="{{ $volume->id }}" @if(old('volume_id', $chapter->volume_id) && old('volume_id', $chapter->volume_id) === $volume->id) selected @endif>
                    {{ $volume->display_name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input type="number" name="chapter_number" id="chapter_number" value="{{ old('chapter_number', $chapter->chapter_number) }}">
    ...
    <div>
        ...
        <input type="text" name="chapter_name" id="chapter_name" value="{{ old('chapter_name', $chapter->chapter_name) }}">
        @error('chapter_name')
            <span>{{ $message }}</span>
        @enderror
    </div>
    ...
    <div>
        ...
        <textarea name="chapter_description" id="chapter_description">
            {{ old('chapter_description', $chapter->chapter_description) }}
        </textarea>
